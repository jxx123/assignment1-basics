...
from cs336_basics import train_bpe, tokenizer, transformer, train, optimizer, data_loader
...
def run_swiglu(
    ...
    """
    swiglu = transformer.SwiGLU(d_model, d_ff=d_ff)
...
def run_multihead_self_attention(
    ...
    """
    mutli_head_attn = transformer.MultiHeadSelfAttention(
        d_model, num_heads)
    mutli_head_attn.load_state_dict({
        'q_proj.weight': q_proj_weight,
        'k_proj.weight': k_proj_weight,
        'v_proj.weight': v_proj_weight,
        'output_proj.weight': o_proj_weight
    })
    return mutli_head_attn(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    return mutli_head_attn(in_features, token_positions=token_positions)
...
def run_rope(
    ...
    """
    rope = transformer.RoPE(theta, d_k, max_seq_len)
...
def run_transformer_block(
    ...
    block = transformer.TransformerBlock(
        d_model, d_ff, num_heads, max_seq_len, theta=theta)
...
def run_rmsnorm(
    ...
    """
    rmsnorm = transformer.RMSNorm(d_model, eps=eps)
...
def run_gradient_clipping(
    ...
    """
    return optimizer.clip_gradient(parameters, max_l2_norm)
...
def run_save_checkpoint(
    ...
    """
    train.save_checkpoint(model, optimizer, iteration, out)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe.train_pipeline(
        input_path, vocab_size, special_tokens, num_processes=10
    )
